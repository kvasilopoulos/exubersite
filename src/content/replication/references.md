...
| Guo, G., Sun, Y. & Wang, S. (2019). "Testing for moderate explosiveness." *The Econometrics Journal*, 22(3), 279-303. | open (institutional access); `root_ci()` was implemented from Skrobotov's secondary restatement before this was found — worth cross-checking the implementation against the primary source now that it's available | **done** — `root_ci()` |
